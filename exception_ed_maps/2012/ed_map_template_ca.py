...
city_council_patt = re.compile(r'(?P<city>[A-Z ]+)(?=\W)\D*(?P<number>\d+)')
ed_map.update(dict(county_council_dicts))

city_council_dicts = []
for cc in city_council:
    cc_orig = cc
    cc = re.sub(r'CITY (OF)?|COUNCIL|DIST(RICT)?','',cc)
    m = city_council_patt.match(cc)
    if not m:
        print 'unmatched cc: ',cc
        continue
    city = m.groupdict()['city'].strip()
    number = int(m.groupdict()['number'])
    city_council_dicts.append(('city of {city} - council district {number}'.format(city=city,number=number).lower(),{'name':cc_orig,'type':'city_council'}))
ed_map.update(dict(city_council_dicts))
city_dict = {}
for rc in rcity:
    city_dict['city of {city}'.format(city=rc.strip()).lower()]={'name':rc,'type':'city'}
ed_map.update(city_dict)
