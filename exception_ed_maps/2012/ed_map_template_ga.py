...
special_1_school_district = d_dict['special_1_school_district']
...
sd_dicts = []
for sd in special_1_school_district:
    county_possibles = []
    for c in county_id:
        if sd.startswith(c):
    ...
    old_district_stuff = old_sd.replace(old_county_name,'').strip().strip('_')
    district_stuff = sd.replace(county_name,'').strip().strip('_')
    if re.match(r'^\d+$', district_stuff):
        for f in sd_fillers:
            sd_dicts.append(('{county_name} {filler} {district_number}'.format(filler=f,county_name=county_name, district_number=clean_county_number(district_stuff)).lower(),{'name':'{county_name}_{district_stuff}'.format(county_name=old_county_name, district_stuff=old_district_stuff),'type':'special_1_school_district'}))
    elif district_stuff.endswith('LRG'):
        sd_dicts.append(('{county_name} {filler}'.format(filler=f,county_name=county_name).lower(),{'name':'{county_name}_{district_stuff}'.format(county_name=old_county_name, district_stuff=old_district_stuff),'type':'special_1_school_district'}))
...
for county in county_council:
    ...
    for f in fillers:
        if m and  m.groupdict()['district_number']:
            num = m.groupdict()['district_number']
            if county_name.lower() == 'jeff davis':
                num = int(num)/100
            county_council_dicts.append(('{county_name} {filler} {district_number}'.format(filler=f,county_name=county_name, district_number=clean_county_number(num)).lower(),{'name':'{county_name}_{district_stuff}'.format(county_name=old_county_name, district_stuff=old_district_stuff),'type':'county_council'}))
        elif district_stuff.endswith('LRG'):
