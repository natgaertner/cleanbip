...
for sd in school_district:
    for f in sd_fillers:
        ...
        if sd == 'LaCrescent-Hokah':
            new_sd = 'La Crescent-Hokah'
        else:
            new_sd=sd
        sd_dicts.append(('{filler} {district_name}'.format(filler=f, district_name=county_name_clean(new_sd)).lower(),{'name':'{district_stuff}'.format(district_stuff=sd),'type':'school_district'}))
